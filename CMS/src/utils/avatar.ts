function md5(str: string): string {
  ...
  const preparedString = utf8Encode(str);
  const x = convertToWordArray(preparedString);
...
export function getMemberAvatarUrl(email?: string, name?: string, photoUrl?: string): string {
  if (photoUrl && photoUrl.trim()) {
    return photoUrl.trim();
  }
  
  const trimmedName = (name || 'Member').trim();
  const defaultFallback = `https://ui-avatars.com/api/?name=${encodeURIComponent(trimmedName)}&background=random&color=fff&size=300`;

  if (email && email.trim()) {
    const cleanEmail = email.trim().toLowerCase();
    const hash = md5(cleanEmail);
    return `https://www.gravatar.com/avatar/${hash}?s=300&d=${encodeURIComponent(defaultFallback)}`;
  }
